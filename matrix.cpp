...
namespace MatrixLib {
    ...
    Matrix::Matrix(int rows, int cols, float initVal = 0) {
    ...
    Matrix::Matrix(const vector<float>& vec, int rowFold = 1) {
        int len = vec.size();
    ...
    Matrix::Matrix(const vector<unsigned char>& vec, int rowFold = 1) {
        int len = vec.size();
    ...
    void Matrix::extendRow(int amount, float initVal = 0) {
        if (matrix.empty()) return;
        while (amount-- > 0) matrix.emplace_back(vector<float>(matrix[0].size(), initVal));
    }
    void Matrix::extendCol(int amount, float initVal = 0) {
        if (matrix.empty()) return;
        for (int r = 0; r < matrix.size(); ++r) {
    ...
    void Matrix::reshape(int newRow, int newCol, float defaultVal = 0) {
    ...
    void Matrix::print2(int eachRows = 10) const {
        cout << "Matrix===========================================" << endl;
        cout << "Rows = " << matrix.size() << " , Cols = " << (matrix.size() ? matrix[0].size() : 0) << endl;
