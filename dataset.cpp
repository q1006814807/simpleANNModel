...
public:
    ...
    void initTrainingDataset();                                 // 只加载训练集
    void initTestDataset();                                     // 只加载测试集
    void init();                                                // 全部加载至内存

    Dataset(int training_amount, int test_amount) {             // 构造函数
    ...
    void loadDataSet(   const string& path_image,               // 加载数据集
    ...
    Matrix& readTrainingImg(int& label, int select);            // 获取一个训练集数据
    Matrix& readTestImg(int& label, int select);                // 获取一个测试集数据

};



void Dataset::initTrainingDataset() {
...
void Dataset::loadDataSet(    const string& path_image, 
                        const string& path_label,
                        const int image_start,
                        const int label_start,
                        const int amount,
...
Matrix& Dataset::readTrainingImg(int& label, int select) {
    label = trainingLabelSet[select];
    return trainingImageSet[select];
}

Matrix& Dataset::readTestImg(int& label, int select) {
